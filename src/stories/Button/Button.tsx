...
export default function Button({
  ...
  loading, type = 'button', ariaLoadingText, fluid = false, ...rest
          ...
          <>
            ...
            <span className={`${styles['btn-text']}`}>
              ...
              {icon && iconPlace === 'right' ? (
                <>
                  <Icon width={6} height={16} color={iconColor || defaultIconColor(variant)} name={icon} aria-label={ariaIconLabel} className={styles['btn-icon-right']} />
                </>
              ) : null}
            </span>
          </>
